import axios from "axios";

/**
 * Service API central — Axios instance configurée.
 * En dev : laisser VITE_API_URL vide pour passer par le proxy Vite (/api → backend).
 */
export const API_BASE_URL = import.meta.env.VITE_API_URL ?? "";
export const USE_MOCK = String(import.meta.env.VITE_ENABLE_MOCK || "").toLowerCase() === "true";

const api = axios.create({
  baseURL: API_BASE_URL || "/api/v1",
  timeout: 20000,
  headers: { "Content-Type": "application/json" },
});

api.interceptors.request.use((config) => {
  const token = localStorage.getItem("airsense_token");
  if (token) config.headers.Authorization = `Bearer ${token}`;

  try {
    const raw = localStorage.getItem("airsense_user");
    if (raw) {
      const user = JSON.parse(raw);
      if (user?.activeCompanyId) {
        config.headers["x-company-id"] = String(user.activeCompanyId);
      }
    }
  } catch {
    // ignore
  }
  return config;
});

api.interceptors.response.use(
  (res) => res,
  (error) => {
    if (error.response?.status === 401) {
      localStorage.removeItem("airsense_token");
      localStorage.removeItem("airsense_user");
      if (!window.location.pathname.startsWith("/login")) {
        window.location.href = "/login";
      }
    }
    return Promise.reject(error);
  }
);

export default api;

export function unwrapApiData(payload) {
  if (payload && typeof payload === "object" && "success" in payload && "data" in payload) {
    return payload.data;
  }
  return payload;
}

export function unwrapApiMeta(payload) {
  if (payload && typeof payload === "object" && "meta" in payload) {
    return payload.meta;
  }
  return null;
}

export function resolveApiErrorMessage(error, fallback) {
  const e = error;
  if (!e.response) {
    return "Backend indisponible. Verifiez que le serveur tourne (port 4000) et VITE_API_URL / proxy Vite.";
  }
  return e.response?.data?.error?.message ?? fallback;
}
